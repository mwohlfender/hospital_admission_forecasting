...
table_days_wastewater <- read_csv(file = Path_Data_Wastewater_Processed)
...
features_target_003_base <- tibble(date_day = sequence_days) %>%
...
# join `features_target_003_base` and `table_days_wastewater`
features_target_003_base <- features_target_003_base %>%
  left_join(table_days_wastewater %>% dplyr::select("date_day", "sars_cov2_rna"), by = "date_day")


# create data sets and store them
data_preparation_series_001(data_features = features_target_003_base,
...
# `features_target_003_base`: add variable y (= y_{0,N} for all N present in `table_parameters_kNp`)
...
# store `features_target_003_base`
path_features_target_003_base <- get_path_features_target(number_xy = number_xy,
                                                          number_combination_features = number_combination_features,
                                                          name_data_set = name_data_set,
                                                          option_path = "long_directory_filename")

if (!(file.exists(path_features_target_003_base)) | Bool_Data_Processing_Do_New) {
  
  write_csv(x = features_target_003_base,
            file = path_features_target_003_base)
