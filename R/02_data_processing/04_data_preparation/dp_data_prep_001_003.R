...
data_features_003_base <- tibble(date_day = sequence_days) %>%
...
# join `data_features_003_base` and `table_days_wastewater`
data_features_003_base <- data_features_003_base %>%
  left_join(table_days_wastewater %>% dplyr::select("date_day", "sars_cov2_rna"), by = "date_day")


# create data sets and store them
data_preparation_series_001(data_features = data_features_003_base,
...
# `data_features_003_base`: add variable y (= y_{0,N} for all N present in `table_parameters_kNp`)
...
# store `data_features_003_base`
path_data_features_003_base <- get_path_data_features(directory_data = paste0(Directory_Data, Subdirectory_Data_Feature_sets),
                                                      number_xy = number_xy,
                                                      number_combination_features = number_combination_features,
                                                      name_data_set = name_data_set)

if (!(file.exists(path_data_features_003_base)) | Bool_Data_Processing_Do_New) {
  
  write_csv(x = data_features_003_base,
            file = path_data_features_003_base)
