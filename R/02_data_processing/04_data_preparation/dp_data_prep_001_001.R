...
data_features_001_base <- tibble(date_day = sequence_days) %>%
...
# join `data_features_001_base` and `table_days_visits_emergency_20200101_20230630`
data_features_001_base <- data_features_001_base %>%
...
data_preparation_series_001(data_features = data_features_001_base,
...
# `data_features_001_base`: add variable y (= y_{0,N} for all N present in `table_parameters_kNp`)
...
# store `data_features_001_base`
path_data_features_001_base <- get_path_data_features(directory_data = paste0(Directory_Data, Subdirectory_Data_Feature_sets),
                                                      number_xy = number_xy,
                                                      number_combination_features = number_combination_features,
                                                      name_data_set = name_data_set)

if (!(file.exists(path_data_features_001_base)) | Bool_Data_Processing_Do_New) {
  
  write_csv(x = data_features_001_base,
            file = path_data_features_001_base)
