...
table_days_visits_emergency_20200101_20230630 <- read_csv(file = Path_Table_Days_Visits_Emergency_20200101_20230630)
...
features_target_001_base <- tibble(date_day = sequence_days) %>%
...
# join `features_target_001_base` and `table_days_visits_emergency_20200101_20230630`
features_target_001_base <- features_target_001_base %>%
  left_join(table_days_visits_emergency_20200101_20230630, by = "date_day") %>%
  dplyr::rename(c("visits_emergency" = "visits"))


# create data sets and store them
data_preparation_series_001(data_features = features_target_001_base,
...
# `features_target_001_base`: add variable y (= y_{0,N} for all N present in `table_parameters_kNp`)
...
# store `features_target_001_base`
path_features_target_001_base <- get_path_features_target(number_xy = number_xy,
                                                          number_combination_features = number_combination_features,
                                                          name_data_set = name_data_set,
                                                          option_path = "long_directory_filename")

if (!(file.exists(path_features_target_001_base)) | Bool_Data_Processing_Do_New) {
  
  write_csv(x = features_target_001_base,
            file = path_features_target_001_base)
