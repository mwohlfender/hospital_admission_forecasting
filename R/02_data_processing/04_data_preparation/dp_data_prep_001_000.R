...
features_target_000_base <- tibble(date_day = sequence_days) %>%
...
data_preparation_series_001(data_features = features_target_000_base,
...
# `features_target_000_base`: add variable y (= y_{0,N} for all N present in `table_parameters_kNp`)
...
for (ii in 1:length(list_N)) {
  ...
  features_target_000_base <- features_target_000_base %>%
    dplyr::mutate(y=x) %>%
...
# store `features_target_000_base`
path_features_target_000_base <- get_path_features_target(number_xy = number_xy,
                                                          number_combination_features = number_combination_features,
                                                          name_data_set = name_data_set,
                                                          option_path = "long_directory_filename")

if (!(file.exists(path_features_target_000_base)) | Bool_Data_Processing_Do_New) {
  
  write_csv(x = features_target_000_base,
            file = path_features_target_000_base)
  
}



# data set "age": stratification by age ----
...
# join `sequence_days` and `table_days_icd_u07_1_20200101_20230630`
features_target_000_age <- tibble(date_day = sequence_days) %>%
  left_join(table_days_icd_u07_1_20200101_20230630 %>%
              dplyr::select(c("date_day", "icd10_code_U07_1", "icd10_code_U07_1_group_age_1",
                              "icd10_code_U07_1_group_age_2", "icd10_code_U07_1_group_age_3",
                              "icd10_code_U07_1_group_age_4", "icd10_code_U07_1_group_age_5")), by = "date_day") %>%
  mutate_if(is.numeric, replace_na, replace = 0) %>%
  dplyr::rename(c("x" = "icd10_code_U07_1",
                  "x_group_age_1" = "icd10_code_U07_1_group_age_1",
                  "x_group_age_2" = "icd10_code_U07_1_group_age_2",
                  "x_group_age_3" = "icd10_code_U07_1_group_age_3",
                  "x_group_age_4" = "icd10_code_U07_1_group_age_4",
                  "x_group_age_5" = "icd10_code_U07_1_group_age_5"))


# create data sets and store them
data_preparation_series_001(data_features = features_target_000_age,
                            number_combination_features = number_combination_features,
                            name_data_set = name_data_set,
                            table_parameters_kNp = table_parameters_kNp_filtered,
                            table_dates_train_test = table_dates_train_test_detailed_filtered,
...
# `features_target_000_age`: add variable y (= y_{0,N} for all N present in `table_parameters_kNp`)
# y_{0,N}(t) = x(t) + x(t+1) + x(t+2) + ... + x(t+N-1)
list_N <- table_parameters_kNp %>% pull(N) %>% unique()
...
for (ii in 1:length(list_N)) {
  ...
  features_target_000_age <- features_target_000_age %>%
    dplyr::mutate(y=x) %>%
    timetk::tk_augment_leads(.value=y,
                             .lags=-seq(from = 0, to = list_N[ii] - 1, by = 1)) %>%
    dplyr::rename_with(.cols=any_of("y_lag0"),
...
# store `features_target_000_age`
path_features_target_000_age <- get_path_features_target(number_xy = number_xy,
                                                         number_combination_features = number_combination_features,
                                                         name_data_set = name_data_set,
                                                         option_path = "long_directory_filename")

if (!(file.exists(path_features_target_000_age)) | Bool_Data_Processing_Do_New) {
  
  write_csv(x = features_target_000_age,
            file = path_features_target_000_age)
