...
data_features_002_base <- tibble(date_day = sequence_days) %>%
...
# join `data_features_002_base` and `table_days_fever`
data_features_002_base <- data_features_002_base %>%
  left_join(table_days_fever, by = "date_day")


# create data sets and store them
data_preparation_series_001(data_features = data_features_002_base,
...
# `data_features_002_base`: add variable y (= y_{0,N} for all N present in `table_parameters_kNp`)
...
# store `data_features_002_base`
path_data_features_002_base <- get_path_data_features(directory_data = paste0(Directory_Data, Subdirectory_Data_Feature_sets),
                                                      number_xy = number_xy,
                                                      number_combination_features = number_combination_features,
                                                      name_data_set = name_data_set)

if (!(file.exists(path_data_features_002_base)) | Bool_Data_Processing_Do_New) {
  
  write_csv(x = data_features_002_base,
            file = path_data_features_002_base)
