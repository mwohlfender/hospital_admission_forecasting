...
data_features_010_base <- tibble(date_day = sequence_days) %>% 
  left_join(table_days_icd_u07_1_20200101_20230630 %>% dplyr::select("date_day", "icd10_code_U07_1"), by = "date_day") %>%
  mutate_if(is.numeric , replace_na, replace = 0) %>%
  dplyr::rename(c("x" = "icd10_code_U07_1"))

# join `data_features_010_base` and `data_features_001_base`
data_features_010_base <- data_features_010_base %>%
  left_join(data_features_001_base, by = "date_day")

# join `data_features_010_base` and `data_features_002_base`
data_features_010_base <- data_features_010_base %>%
  left_join(data_features_002_base, by = "date_day")

# join `data_features_010_base` and `data_features_005_base`
data_features_010_base <- data_features_010_base %>%
  left_join(data_features_005_base, by = "date_day")

# join `data_features_010_base` and `data_features_006_base`
data_features_010_base <- data_features_010_base %>%
  left_join(data_features_006_base, by = "date_day")


# create data sets and store them
data_preparation_series_001(data_features = data_features_010_base,
...
# `data_features_010_base`: add variable y (= y_{0,N} for all N present in `table_parameters_kNp`)
...
# store `data_features_010_base`
path_data_features_010_base <- get_path_data_features(directory_data = paste0(Directory_Data, Subdirectory_Data_Feature_sets),
                                                      number_xy = number_xy,
                                                      number_combination_features = number_combination_features,
                                                      name_data_set = name_data_set)

if (!(file.exists(path_data_features_010_base)) | Bool_Data_Processing_Do_New) {
  
  write_csv(x = data_features_010_base,
            file = path_data_features_010_base)
