...
table_days_icd_codes <- read_csv(file = Path_Table_Days_ICD_Freq_Codes_Cat_Chap_Covid_20200101_20230630) %>%
  dplyr::select(c("date_day"), starts_with("icd10_cat_"))
...
data_features_007_base <- tibble(date_day = sequence_days) %>%
...
# join `data_features_007_base` and `table_days_icd_codes`
data_features_007_base <- data_features_007_base %>%
  left_join(table_days_icd_codes, by = "date_day")


# create data sets and store them
data_preparation_series_001(data_features = data_features_007_base,
...
# `data_features_007_base`: add variable y (= y_{0,N} for all N present in `table_parameters_kNp`)
...
# store `data_features_007_base`
path_data_features_007_base <- get_path_data_features(directory_data = paste0(Directory_Data, Subdirectory_Data_Feature_sets),
                                                      number_xy = number_xy,
                                                      number_combination_features = number_combination_features,
                                                      name_data_set = name_data_set)

if (!(file.exists(path_data_features_007_base)) | Bool_Data_Processing_Do_New) {
  
  write_csv(x = data_features_007_base,
            file = path_data_features_007_base)
